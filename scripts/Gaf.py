...
def overlap(x, y, a, b):
        """Takes four numbers representing ranges (in numerical order), returns False if they don't overlap"""
        if y< a or x>b:
                return False
        return True
...
#            print featureLength, blockList[blockIdx].featureEnd,blockList[blockIdx].featureStart
#	    print compositeLength, blockList[blockIdx].compositeEnd, blockList[blockIdx].compositeStart
            assert (featureLength == compositeLength), "%d is not %d" % (featureLength, compositeLength)
        return(blockList)
    ...
    def featureToComposite(self, feat, comp, inferStrand=True,
                           mergeAdjacent=False, fullBlocksOnly=False, debug=False,):
        ...
        self.alignFeatureToComposite(feat, comp, strand,
                                     mergeAdjacent=mergeAdjacent, fullBlocksOnly=fullBlocksOnly, debug=debug)
    ...
    def alignFeatureToComposite(self, feat, comp, strand,
        ...
        for fb in featureBlocks:
            ...
            for cb in compositeBlocks:
                if fullBlocksOnly:
                    if (fb.compositeStart == cb.compositeStart and fb.compositeEnd == cb.compositeEnd): 
                        block = AlignmentBlock(fb.featureStart, fb.featureEnd,
                                           cb.featureStart, cb.featureEnd)
                        featureToCompositeBlocks.append(block)

                elif fb.compositeStart <= cb.compositeEnd \
