...
class Transcript(object):
	...
	self.FeatureType = fields[2]
	self.CompositeType = fields[8]
    ...
    def makeJunctionGTF(self):
        """Creates junctions in GTF format"""
        self.exonGTF = []
        lastfield = "gene_id \"{0}\"; transcript_id \"{1}\";".format(self.Gene, self.FeatureID)
        head = ("\t").join([self.chr, 'GAF3', 'exon'])
        tail = ("\t").join(['.', self.strand, '.'])
        for e in self.exons:
            outline = ("\t").join([head, e, e, tail, lastfield])
            self.exonGTF.append(outline)
    def makeORFGTF(self):
        """Creates exons in GTF format"""
        self.orfGTF = []
        ...
        tail = ("\t").join(['.', self.strand])
        for e in sorted(self.exonTable, key=lambda f: f.start):    
